...
[Kakarot zkEVM](https://kakarot.org) is an implementation of the [Ethereum Virtual Machine](https://ethereum.org/en/developers/docs/evm/) (EVM) written in Cairo. Cairo is a Turing-complete language associated with the CairoVM. The CairoVM enables provable compute by leveraging polynomials and the [ZK-STARK proof system](https://starkware.co/stark/). A zkEVM is characterised by its ability to generate provable transactions and, thus, provable blocks. With Kakarot being built on top of the CairoVM, every transaction executed on Kakarot is provable.
