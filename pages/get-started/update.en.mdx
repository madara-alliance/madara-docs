...
In this section, we will guide you through the process of updating your Madara client to the latest version, ensuring you can continue to query the Starknet blockchain smoothly.

We want anyone to be able to update a Madara full node, which is why we've divided this section into 3 difficulty levels:

- [**Low-level**](#low-level-update-from-source) (updating from source by pulling the latest code and rebuilding the Rust binary locally)
- [**Mid-level**](#mid-level-update) (updating using Docker **recommended** via the latest Docker images)
- [**High-level**](#high-level-update-clicknplay) (updating using a high-level interactive menu)
...
Now that you know how to update your Madara client, you might want to set some parameters in order to customize it.
