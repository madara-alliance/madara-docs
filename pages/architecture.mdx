...
Many of the existing blockchain stacks are just open source versions of the L2 chain. While this ensures you've something which has been tested in production, it
...
We didn't want this to be the case with Madara. We wanted to build a stack for app chain developers which was not only secure but also flexibile. As a result, for the Madara stack,
...
was always designed to be flexibile so that you could build any chain using it. You can see the 150+ chains that are built on top of it [here](https://parachains.info/).

## Forking Substrate

While Substrate is definitely powerful, there are some parts which can modified to make the stack work better for L2s/L3s. Particularly, we have [forked Substrate](https://github.com/massalabs/polkadot-sdk/tree/release-polkadot-v1.3.0-std)
...
1. Making the runtime `std` compatible: We currently rely on multiple Starknet libraries ([blockifier](https://github.com/starkware-libs/blockifier),
   ...
   is also compiled to `WASM` (which is why it needs to be `no-std` compatible) which also slows down the perfomance by ~2x. As a result, after analyzing the tradeoff, we decided to fork away Substrate
...
The above architecture diagram shows all the mages components of Madara. Below, we describe the functionalities of each component and also walthrough the lifecycle of a transaction.

##### JSON RPC Client

When a tranaction is first created, it reaches the JSON RPC client inside the [mc-rpc](https://github.com/keep-starknet-strange/madara/blob/main/crates/client/rpc/src/lib.rs) (mc = madara-client) crate.
...
[Pallets](https://docs.substrate.io/reference/frame-pallets/) are a Substrate term which refer to modules which can be used to add business logic to your runtime. User transactions usually trigger some function
inside a pallet. The [Starknet pallet](https://github.com/keep-starknet-strange/madara/blob/main/crates/pallets/starknet/src/lib.rs) is the core component of Madara where majority of the exeuction and storage happens.
As shown in the diagram above, the validatioon and execution logic are defined inside the Starknet pallet.

##### Blockifier

The [blockifier](https://github.com/starkware-libs/blockifier) is the component used by the Starknet sequencer in production for executing transactions and creating blocks. For the purpose of Madara, the block building logic
is native to the framework. However, we still import the blockifer to execute Cairo contracts.
...
checks if the transaction is correct and can be added to the mempool. This involves nonce and signature checks. Since Madara supports account abstraction, accounts in Madara can have custom validation logics -
...
While the method is called `validate_unsigned`, the transaction actually contains the signature. As we support account abstract, we don't use the default validation logic of Substrate based on public and private keys. Instead, we keep Starknet
...
If the transaction passes validation, it's sent to the mempool and the transaction hash is sent back to the user. If the validation fails, an error is propogated back to the user.
...
Starknet has 4 write transactions and the exection logic for all of them is defined in the the starket pallet

1. **Invoke transactions**: These are the most common transactions which are used to interact with existing smart contracts or deploy new contracts. [Code](https://github.com/keep-starknet-strange/madara/blob/7900c0b884ee3fb978711ff65b19e98f0c143c03/crates/pallets/starknet/src/lib.rs#L487).
2. **Deploy account transactions**: These are special transactions which are used to deploy accounts on the chain. As mentioned, Madara supports account abstraction which means your account needs to be deployed before it can be used. However, most wallets
   abstract this part from the user and you don't even realise when your wallets get deployed! [Code](https://github.com/keep-starknet-strange/madara/blob/7900c0b884ee3fb978711ff65b19e98f0c143c03/crates/pallets/starknet/src/lib.rs#L598).
3. **Declare transactions**: These are used to declare new contracts which can later be deployed using `INVOKE` (for normal smart contrats) and `DEPLOY_ACCOUNT` (for account contracts) transactions. [Code](https://github.com/keep-starknet-strange/madara/blob/7900c0b884ee3fb978711ff65b19e98f0c143c03/crates/pallets/starknet/src/lib.rs#L539).
...
4. Executes the transactions and makes the required storage changes.
   1. For transaction exeucution, we pass the details to the blockifier which calls the VM internally
   2. For storage changes, we call the [blockifier state adapter](https://github.com/keep-starknet-strange/madara/blob/main/crates/pallets/starknet/src/blockifier_state_adapter.rs). This is a `struct` which implements the `StateChanges` trait from the blockifier. The blockifier takes this as an input when executing
...
learn about blocks, transactions and other details but as they don't live inside the runtime itself, they don't slow down block production. Madara runs multiple offchain wokers as described below
...
When you send a message from a L2 to the L1 (or from a L3 to the L2), the settlement worker is also responsible for pushing that message.
...
be used to add an other DA layer.

![da_worker](../static/img/da_worker.png)

After the state diffs worker has finished building the state diffs, they are propogated to the DA worker. The DA worker then creates a proof for these state diffs if needed and submits the data to the respective DA layer.
...
This block digest is then consumed by the the worker over [here](https://github.com/keep-starknet-strange/madara/blob/7900c0b884ee3fb978711ff65b19e98f0c143c03/crates/client/mapping-sync/src/sync_blocks.rs#L25) and stored in the mapping DB in the form of a
